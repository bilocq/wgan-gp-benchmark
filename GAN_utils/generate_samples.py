...
def generate_image(im_name, data, path, ext='jpg', grid_columns=16, dpi=300):
    """
    ...
    grid = utils.make_grid(data, nrow=grid_columns, padding=1)
    plt.imshow(grid.cpu().numpy().transpose((1, 2, 0)))
    plt.axis('off')
    plt.savefig(os.path.join(path,'{}.{}'.format(im_name, ext)), dpi=dpi)
    plt.close()
