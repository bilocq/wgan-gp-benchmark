...
def main(args, device):    
    ...
    img_noise = torch.randn(args.num_img, generator.latent_dim, device=device)      # Used to generate sample images
    ...
    if resume:
        fid_dict = dict(filter(lambda item: item[1] is not None, metrics_frame['FID'].items()))
        best_iter = min(fid_dict, key=fid_dict.get)
        best_fid = fid_dict[best_iter]
        logger.log(1, f'Previous best fid = {best_fid}. Obtained after {best_iter} iterations.\n')
    else:
    ...
    for i in range(len(checkpoints)):
        ...
        with torch.no_grad():
            ...
            generate_samples.generate_image(f'iter_{current_iter}', 
                                            gen_imgs, 
                                            img_folder, 
                                            ext=args.img_ext, 
...
if __name__ == '__main__':
    ...
    # Generated images
    parser.add_argument('--num_img',      type=int, default=64,    help='Number of individual samples in image grid produced at each checkpoint. Must be > 0.')
    parser.add_argument('--grid_columns', type=int, default=16,    help='Number of columns in image grid produced at each checkpoint. Must be > 0.')
    parser.add_argument('--img_ext',      type=str, default='jpg', help='File extension for image grid produced at each checkpoint.')
    parser.add_argument('--img_dpi',      type=int, default=3000,  help='dpi value for image grid produced at each checkpoint. Larger value means higher resolution. Must be > 0')
